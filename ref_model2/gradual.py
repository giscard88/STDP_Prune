...
for ts in xrange(400):
    ...
    weights_p.append(numpy.mean(numpy.array(p_p)))
    weights_np.append(numpy.mean(numpy.array(np_p)))
...
sim_len=str(int(50*400))

fn_comm=str(top_down_pyr)+'_'+str(top_down_pv)+'_'+str(msd)+'_'+str(fraction)+'_'+sim_len+'.json'



weights={'p_p':weights_p, 'np_p':weights_np}
fp=open('trace_weights_'+fn_comm,'w')
