...
nest.CopyModel('stdp_synapse','IC_bound',{"receptor_type":1,'Wmax':25.0})
...
syn_dict={'model':'IC_bound','weight':td_super_pyr,'delay':IC_delay}
target_list=[]
...
conn_dict={'rule':'pairwise_bernoulli','p':0.3}
syn_dict={'model':'IC_bound','weight':td_super_pv,'delay':IC_delay}
target_list=[]
source_list=All_cells['LMl23pyr'][0]
...
source_list=list(All_cells['LMl23pyr'][0])
target_list1=list(All_cells['V1_1l23pyr'][0])
target_list2=list(All_cells['V1_2l23pyr'][0])

pref_pref_td=nest.GetConnections(source_list,target_list1)
nonpref_pref_td=nest.GetConnections(source_list,target_list2)
...
pref_pref_td_weight=nest.GetStatus(pref_pref_td)
nonpref_pref_td_weight=nest.GetStatus(nonpref_pref_td)
p_p=[]
np_p=[]
...
for xin in pref_pref_td_weight:
    td_p_p.append(xin['weight'])
for xin in nonpref_pref_td_weight:
    td_np_p.append(xin['weight'])

print 'initial weights'
print 'pref_pref', numpy.mean(numpy.array(p_p))
print 'nonpref_pref', numpy.mean(numpy.array(np_p))

print 'top-down pref_pref', numpy.mean(numpy.array(td_p_p))
print 'top-down pref_nonpref', numpy.mean(numpy.array(td_np_p))


weights['before_p_p']=p_p
weights['before_np_p']=np_p

weights['before_td_p_p']=td_p_p
weights['before_td_np_p']=td_np_p
...
weights['after_td_p_p']=td_p_p
weights['after_td_np_p']=td_np_p
