...
mm2=nest.Create('multimeter', params={'record_from': ['I_syn_1','I_syn_2'],'to_accumulator':True})
...
# IC 
conn_dict={'rule':'pairwise_bernoulli','p':0.0}
syn_dict={'model':'IC','weight':0.0,'delay':IC_delay}
...
lfp2={}
lfp2_raw=numpy.abs(i21)+numpy.abs(i22)
lfp2['time']=list(t2)
lfp2['lfp']=list(lfp2_raw) # no vip cells
...
fp=open('spikes_no_bottom-up_'+fn_comm,'w')
json.dump(spikes,fp)
fp.close()


fp=open('lfp_no_bottom-up_'+fn_comm,'w')
json.dump(lfp,fp)
fp.close()


fp=open('lfp2_no_bottom-up_'+fn_comm,'w')
json.dump(lfp2,fp)
fp.close()
